...
class PriorityQueue(Container):
    ...
    def add(self, item):
        ...
        for index in range(len(self._items)):
            ...
            # If <item> has a higher priority than the current item at <index>
            # then we put it in front of the item in the queue.
            if item < self._items[index]:
                self._items.insert(index, item)
                return
            # Otherwise, go to the next element.

        # If it has gone through the list without inserting item somewhere, then
        # it has the lowest priority and it should be appended to the end of
        # self._items
